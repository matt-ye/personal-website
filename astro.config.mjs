  ...
  return {
    ...
    hooks: {
      'astro:build:done': ({ dir, logger }) => {
        ...
        const slugs = [...src.matchAll(/slug:\s*'(week-(\d{2})[^']*)'/g)];
        if (slugs.length === 0) { logger.warn('no week slugs found, skipped'); return; }

        let pages = 0, links = 0;
        // 中文（dist/projects/…）與英文（dist/en/projects/…）各處理一輪：
        // 目錄、網址前綴、以及「該語言版本存在與否」都是每個語言自己的事——
        // 英文頁只連英文頁，讀者不該點一條內文連結就被丟回中文版。
        // 沒翻譯的週次在英文那輪 statSync 不到，自然保持純文字。
        for (const prefix of ['', '/en']) {
          const courseDir = join(root, ...prefix.split('/').filter(Boolean), ...COURSE.split('/'));
          ...
          for (const m of slugs) {
            ...
            try {
              ...
              urlByWeek.set(week, `${prefix}/${COURSE}/${slug}/`);
            } catch { /* 目標頁不在 dist，不連 */ }
          }
          if (urlByWeek.size === 0) continue;
          ...
          for (const entry of readdirSync(courseDir)) {
            ...
            // 中文頁寫「W12」「第 12 週」，英文頁多了「Week 12」（翻譯把「第 12 週」
            // 換成這個形式）；範圍寫法「W19–20」「W14–18」「第 21–23 週」「Weeks 19–21」
            ...
            // 導覽列的品牌字「WEEK 26」是全大寫，比對區分大小寫所以不會誤中。
            const linkify = (text) =>
              text.replace(
                /\bW(?<ra>\d{1,2})(?<rsep>\s?[–—-]\s?)(?<rb>\d{1,2})\b|\bWeeks?\s(?<wra>\d{1,2})(?<wrsep>\s?[–—-]\s?)(?<wrb>\d{1,2})\b|第(?<ds1>\s?)(?<da>\d{1,2})(?<dsep>\s?[–—-]\s?)(?<db>\d{1,2})(?<ds2>\s?)週|\bW(?<sa>\d{1,2})\b|\bWeek\s(?<wa>\d{1,2})\b|第\s?(?<sb>\d{1,2})\s?週/g,
                (...args) => {
                  const whole = args[0], g = args[args.length - 1];
                  if (g.ra) return wrap('W' + g.ra, +g.ra) + g.rsep + wrap(g.rb, +g.rb);
                  if (g.wra) {
                    const head = whole.slice(0, whole.length - (g.wra + g.wrsep + g.wrb).length) + g.wra;
                    return wrap(head, +g.wra) + g.wrsep + wrap(g.wrb, +g.wrb);
                  }
                  if (g.da) return '第' + g.ds1 + wrap(g.da, +g.da) + g.dsep + wrap(g.db, +g.db) + g.ds2 + '週';
                  if (g.sa) return wrap(whole, +g.sa);
                  if (g.wa) return wrap(whole, +g.wa);
            ...
            if (pageLinks > 0) {
              ...
              // （第一個 </style> 是內容片段自帶的頁面專屬樣式，中英文頁結構相同）。
        ...
        logger.info(`linked ${links} week reference(s) across ${pages} course page(s) (zh+en)`);
      },
    },
  };
}
