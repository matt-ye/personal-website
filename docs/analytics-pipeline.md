...
> **不需要的參照連結**（GA4 後台設定，非程式碼）：新資料串流 `15435291446` 建立時
> 這份清單是空的，owner 已於 **2026-08-14 補上 `ecpay.com.tw` 與 `polar.sh`**。
>
> 為什麼要設：捕夢網付款會導向這兩個網域再回到 success 頁，不排除的話回導會被
> 算成新 session、來源記為付款商，**購買轉換的歸因會錯**。
>
> 兩件之後會再遇到的事：
> - 這個設定是**串流層級**的，**不會從舊串流繼承**——日後再建新串流要重設一次
> - 它**只對設定之後的流量生效，不回溯**。所以新資源在 2026-08-14 之前的流量
>   來源報表可能把付款商列為參照來源，那是已知的、不會再變的歷史，不用去查
